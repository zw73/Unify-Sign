  ...
  this.init = function () {
    config.supported_signs.forEach(taskInfo => {
      ...
      if (!signTaskService.queryTaskExists(taskInfo.taskCode)) {
        ...
        let id = signTaskService.updateTaskInfoByTaskCode(dbTaskInfo)
        logUtils.debugInfo(['更新任务信息：{} {} taskId: {}', dbTaskInfo.taskCode, dbTaskInfo.taskName, id])
      }
      if (taskInfo.subTasks && taskInfo.subTasks.length > 0) {
        taskInfo.subTasks.forEach(subTaskInfo => {
          let dbSubTaskInfo = {
            ...
            enabled: subTaskInfo.enabled,
          }
          if (signTaskService.queryTaskExists(taskInfo.taskCode + ':' + subTaskInfo.taskCode)) {
            let id = signTaskService.updateTaskInfoByTaskCode(dbSubTaskInfo)
            logUtils.debugInfo(['更新子任务信息：{} {} taskId: {}', dbSubTaskInfo.taskCode, dbSubTaskInfo.taskName, id])
          } else {
